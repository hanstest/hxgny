...
import { Grid, Table, Header, Modal, Button, Icon, Input, Checkbox } from 'semantic-ui-react'
import { insertSemester, updateSemester, removeSemester } from '../../api/data/methods'

const NEW_DATA_ID = 'NEW_DATA_ID'

class SemesterList2 extends React.Component {
  state = {
    editing: null,
    update: null,
    itemId: null,
  ...
  toggleCurrent = (e, { name, checked }) => {
    e.preventDefault()
    
    const newUpdate = Object.assign(this.state.update || {}, { current: event.target.value })
    this.setState({ update: newUpdate })
  }
  ...
  handleInsertItem = (item) => {
    ...
    insertSemester.call(itemToInsert, (error) => {
      if (error) {
...
SemesterList2.propTypes = {
  items: React.PropTypes.array,
}

export default SemesterList2
